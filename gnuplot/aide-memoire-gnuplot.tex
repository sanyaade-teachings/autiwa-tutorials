...
reset
\end{verbatim}
Cette partie là est plus ou moins une boite noire. La ligne \texttt{set dgrid3d} dans le cas d'un fichier de données (contrairement à une fonction que l'on définierait directement dans gnuplot ($f(x,y) = x*x - y*y$ par exemple). On stocke alors les valeurs des contours dans un fichier \textbf{contour.dat}, puis \textbf{contour1.dat} (pour un fichier qui présente des doubles lignes, apparemment nécessaires à l'affichage correct des contours pour la suite).
