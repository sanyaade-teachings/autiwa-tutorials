...
L'opérateur d'élévation à la puissance se note "**". L'expression \emph{a**b} correspond à la notation mathé\-ma\-tique $a^{b}$. 

Le résultat de l'expression \emph{a**b} est entier si \emph{a} et \emph{b} sont entiers, sinon le résultat est réel.

Soit \emph{b} un entier positif, 
\begin{align}
...
La variable $y$ est de type \emph{logical}. Les parenthèses ne sont 
pas obligatoires mais facilitent la lecture (notez les points 
obligatoires de part et d'autre de \emph{not},\emph{and} et \emph{or}).
...
\end{lstlisting}
\emph{nb} et \emph{PI} sont alors des constantes symboliques dont les valeurs ne peuvent pas être modi\-fiées durant le programme (le compilateur affiche un message d'erreur si il trouve dans le corps du programme l'instruction \emph{nb = 12} par exemple).
...
La forme la plus générale du \emph{if} structuré peut être 
...
\end{lstlisting}
où \emph{exp\_log} est une expression quelconque de type \emph{logical} (par exemple : \emph{if (a $>$= 0) then} ), \emph{bloc} est un bloc d'instruc\-tions, \mbox{[ ]} signifie que le contenu est facultatif, \mbox{[ ]\ldots} signifie que le contenu peut apparaître plusieurs fois. Dans l'exemple ci-dessus, si l'expression \emph{exp\_log1} est vraie alors la suite d'instruc\-tions \emph{bloc1} est exécutée. 

Sinon, si l'expression \emph{exp\_log2} est vraie alors c'est la suite d'instruction \emph{bloc2} qui est exécutée (et ainsi de suite). Enfin, si toutes les expressions précédentes (\emph{exp\_log1}, \emph{exp\_log2}, \ldots) sont fausses et si l'instruction \emph{else} est présente, la suite d'instructions \emph{blocn} est exécutée. Si l'instruc\-tion \emph{else} est absente, il est possible qu'aucune instruction ne soit exécutée par un bloc \emph{if}.

\begin{remarque}
Il est recommandé d'indenter (décaler les blocs d'instructions vers la droite d'un certain nombre de caractères blancs) les différents \emph{if} afin d'assurer cette lisibilité. 
\end{remarque}



Un exemple d'utilisation du \emph{if} structuré est donné dans l'exemple ci-après. 
\begin{lstlisting}[language=Fortran]
program nom_if
...
\end{lstlisting}
où \emph{exp\_scal} est une expression de type \emph{integer}, \emph{logical} ou \emph{character}. \emph{selecteur} est une valeur, un intervalle de valeurs ou une liste de valeurs de même type que \emph{exp\_scal}.

Cette instruction permet d'exécuter la suite d'instructions \emph{bloc1} lorsque la valeur de l'expression \emph{exp\_scal} est égale au \emph{selecteur} (ou dans l'intervalle donné par le \emph{selecteur}). Les intervalles sont de la forme suivante : \emph{[valeur1]:valeur2} ou \emph{valeur1:[valeur2]} (par exemple \emph{case(1:)} signifie que l'on s'intéresse aux valeurs entières comprises entre 1 et 2147483647. Les sélecteurs peuvent faire appel à des expressions constantes. Les valeurs figurant dans les différents sélecteurs d'une même instruction \emph{select case} ne doivent pas se recouper (cela engendre une erreur à la compilation).
...
La variable de contrôle \emph{var} est de type \emph{integer} ainsi que les expressions \emph{debut}, \emph{fin} et \emph{pas}.
...
Il est imprudent de chercher à exploiter la valeur de \emph{var} après l'exécution de la boucle \emph{do}. En effet, celle-ci ne prend pas nécessairement la valeur \emph{fin} comme on pourrait le penser a priori.
\end{attention}
...
L'instruction \emph{exit} permet de sortir d'une boucle de façon anticipée. Dans l'exemple suivant, les blocs \emph{bloc1} et \emph{bloc2} sont exécutés pour \emph{var} allant de \emph{debut} à \emph{fin} tant que l'expression logique \emph{exp\_log} est fausse. 

La boucle est interrompue si $var=fin$ ou si \emph{exp\_log} est vraie. Dans le premier cas on passe au \emph{bloc3}. Dans le second cas, \emph{bloc1} est exécuté mais pas \emph{bloc2}. Ensuite, on continue la boucle \emph{do while} tant que \emph{.not.fini} est vrai.

Comme on le voit sur cet exemple, lorsqu'une instruction \emph{exit} appara\^{i}t dans une boucle qui est imbriquée dans une autre boucle, elle met fin à la boucle la plus interne.
\begin{lstlisting}[language=Fortran]
do while (.not.fini)
    do var = debut, fin 
        bloc1     ! bloc d'instructions 
...
\end{lstlisting}
Lorsque \emph{exp\_log} est vraie, on sort de la boucle \emph{do var}


Dans l'exemple suivant, l'instruction \emph{exit} s'applique à la boucle \emph{do while} gr\^ace à l'utilisation de l'identificateur \emph{boucle\_principale}. Ainsi, si \emph{exp\_log} est vraie, ni \emph{bloc2}, ni \emph{bloc3} ne sont exécutés et on sort de la boucle \emph{do while}.
\begin{lstlisting}[language=Fortran]
do while (.not.fini)
         do var = debut, fin 
           bloc1     ! bloc d'instructions 
...
\end{lstlisting}
Lorsque \emph{exp\_log} est vraie, on sort de la boucle principale \emph{do 
while}

L'instruction \emph{cycle} permet de modifier le déroulement normal d'une boucle. Dans l'exemple suivant, \emph{bloc1} et \emph{bloc2} sont exécutés pour \emph{var} allant de \emph{debut} à \emph{fin} par pas de 1. 
...
On peut aussi écrire : \emph{real :: vect\_1(nb)}.
\end{remarque}
...
L'initialisation d'un tableau de rang 1 à \emph{n} éléments est possible en utilisant une liste de \emph{n} éléments définie par \emph{elem\_1,\ldots,elem\_n}. Le tableau correspondant s'écrit \emph{(/elem\_1,\ldots,elem\_n/)}. 

Un tableau à plusieurs dimensions ne peut pas être initialisé directement. Il faut définir un tableau à une dimension et utiliser une fonction particulière qui n'est pas présentée dans ce cours : la fonction \emph{reshape}.
...
w(2:4) = v(7:9)
\end{lstlisting}
les valeurs  \emph{v(7)}, \emph{v(8)}, \emph{v(9)} du tableau \emph{v} sont affectées respectivement aux éléments \emph{w(2)}, \emph{w(3)} et \emph{w(4)} du tableau \emph{w}. Ainsi, la notation \emph{w(i:j)} signifie que l'on s'intéresse aux éléments \emph{w(i), w(i+1), \ldots, w(j)} (\emph{j}$>$\emph{i}). 

\begin{remarque}
Si on écrit \emph{w(3:1) = 1}, aucune affectation ne sera effectuée.
\end{remarque}
...
Il existe en \emph{Fortran 90} des fonctions spécifiques aux tableaux. Les plus usuelles sont \emph{sum} qui fournit la somme des éléments d'un tableau, \emph{maxval} qui donne la valeur maximale d'un tableau, \emph{minval} qui donne la valeur minimale et \emph{product} qui donne le produits des éléments. Les fonctions \emph{dot\_product} et \emph{matmul} permettent d'obtenir respectivement le produit scalaire et le produit matriciel de deux tableaux.
...
Lorsque \emph{bloc1} ne contient qu'une seule instruction et que \emph{bloc2} est absent, on peut utiliser une forme simplifiée identique au \emph{if} logique. 
...
rang du tableau et en utilisant l'attribut \emph{allocatable} 
(allouable). 
\begin{lstlisting}[language=Fortran]
...
L'allocation d'un emplacement se fait en utilisant l'instruction \emph{allocate} en précisant chaque étendue :
\begin{lstlisting}[language=Fortran]
! declaration d'un tableau dynamique d'entiers de rang 2 
real, dimension(:,:), allocatable :: matrice
integer :: n, m, verif
...
\begin{remarque}
Pour vérifier que l'allocation (ou la désallocation) s'est bien effectuée, on peut utiliser l'option \emph{stat}. 
\begin{lstlisting}[language=Fortran]
allocate(matrice(n,m), stat=verif)
\end{lstlisting}
\emph{verif} est une variable entière. Si \emph{verif = 0}, l'allocation s'est bien effectuée, sinon une erreur s'est produite. 
\end{remarque}

Lorsqu'un tableau dynamique devient inutile, il est recommandé de libérer la place allouée à ce tableau. Pour cela, on utilise l'instruction \emph{deallocate} (désallouer).
\begin{lstlisting}[language=Fortran]
...
Le module peut contenir un ensemble de déclarations et d'affectations et/ou une ou plusieurs procédure(s). Dans ce dernier cas, les procédures doivent être précédées par l'instruction \emph{contains}. Un module commence par l'instruction \emph{module} suivi du nom du module et se termine obligatoirement par \emph{end module}.
...
L'utilisation des modules est très simple ; depuis le programme principal, l'appel du module se fait par l'instruction \emph{use} suivi du nom du module. Il faut noter que le nom du module doit être différent de celui du programme principal. L'instruction \emph{use} doit précéder toute autre déclaration.
...
\begin{remarque}
Un module ne doit pas se référencer lui-même, même de manière indirecte. Par exemple, si le module \emph{module1} contient \emph{use module2}, ce dernier ne doit pas contenir l'instruction \emph{use module1}.
\end{remarque}

\subsection{Accès à tout ou partie d'un module}

Une unité de programme qui appelle un module (via l'instruction \emph{use}) à accès à toutes les entités de ce module (variables, procédures). Il est possible cependant de contrôler l'accès à ces entités pour empêcher les conflits entre différentes unités de programme.
...
La première méthode pour contrôler l'accès à un module consiste à protéger certaines entités. Pour cela on utilise les instructions \emph{private} et \emph{public}. Par défaut, l'option d'accès au module est \emph{public}. Toutes les instructions du module se trouvant après l'instruction \emph{private} seront d'accès privé (et donc inaccessible par le programme qui appelle le module). Il est possible aussi d'ajouter l'attribut \emph{private} lors de la déclaration d'une variable pour protéger son accès.
...
Les variables \emph{T}, \emph{m}, \emph{g} et \emph{Po} ne sont pas accessibles par le programme \emph{acces} malgré l'appel du module \emph{module\_atmosphere}. Elles ont, en effet, l'attribut \emph{private} dans \emph{module\_atmosphere}. La variable \emph{h} de la fonction \emph{pression} n'est pas en conflit avec la variable \emph{h} de \emph{module\_constantes} gr\^ace à la restriction d'accès via l'instruction \emph{only}.

\subsubsection{Protection interne}
La seconde méthode de contrôle d'accès entre le module et l'unité de programme qui l'appelle consiste à restreindre l'accès à certaines entités depuis l'unité de programme. Cette restriction s'effectue en utilisant l'attribut \emph{only} lors de l'appel du module (voir le module \emph{module\_atmosphere}).
\begin{lstlisting}[language=Fortran]
...
\section{procédures (fonction et subroutine)}
%TODO page 48
