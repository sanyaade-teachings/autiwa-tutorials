...
\begin{lstlisting}[language=Fortran]
if (var.eq.0) then
  stop "var equal 0, and the program will crash because of that"
end if
\end{lstlisting}
On peut choisir, soit de donner un code retour, en spécifiant un entier, soit on peut spécifier un texte pour décrire l'erreur (auquel cas le code retour sera 0).

\bigskip

Ce que je fais, c'est que les codes de 1 à 9 sont pour le programme principal, les codes 10 à 99 pour les sous programmes, et les codes de 100 à 999 pour les modules qui peuvent être utilisés dans plusieurs codes différents.

Dans la pratique, je trouve plus intéressant de détailler l'erreur avec un texte, même s'il est vrai que ça doit être plus propre de spécifier un code retour avec un entier.
...
\subsection{Apparition de fichiers fort.10}
