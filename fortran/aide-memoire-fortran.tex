...
\subsection{Les expressions logiques avancées}
On peut aussi tester des choses plus compliquées, comme comparer deux à deux tous les éléments de deux listes. 

À ce titre, les commandes \textbf{any()} et \textbf{all()} sont très puissantes.

Par exemple :
\begin{verbatim}
if (any(my_array(:).eq.species)) then
  write(*,*) "at least one is equal to species"
endif
\end{verbatim}

\begin{verbatim}
if (all(my_array(:).eq.my_other_array(:))) then
  write(*,*) "at least one is equal to species"
endif
\end{verbatim}
...
Mais on peut souhaiter utiliser des formats spécifiques, pour limiter le nombre de chiffres significatifs par exemple afin de rendre l'information plus lisible. Voir \refsec{sec:formats} pour plus de détails.

\bigskip

Lorsqu'on ouvre un fichier, plusieurs options sont possibles. En particulier, l'option \gras{status}, qui peut prendre les valeurs suivantes :
\begin{itemize}
...
L'option \gras{form} permet de choisir d'écrire un ASCII ou en format binaire (les formats binaires, beaucoup plus rapides à lire et à écrire peuvent permettre de gagner du temps et de la place quand ces fichiers ne sont pas censés être lus par quelqu'un d'autre qu'un programme. L'option \gras{form} peut prendre les valeurs suivantes : 
\begin{itemize}
\item 'FORMATTED'
\item 'UNFORMATTED'
\end{itemize}

L'option \gras{access} permet d'ajouter le texte à la suite du fichier déjà existant (il y a d'autres options, mais comme je ne les connais pas, je préfère de pas en parler : 
\begin{verbatim}
position='append'
\end{verbatim}
...
\begin{remarque}
Si on souhaite afficher, par exemple, un entier avec le moins de charactères possibles (quand on ne connait pas la longueur de l'entier), on peut utiliser : 
\begin{verbatim}
write(*,'(i0)') nb_sample
\end{verbatim}
\end{remarque}
...
\subsection{Réécrire la ligne précédente dans le terminal : progression}
Il est possible, comme dans d'autres langages de réécrire la dernière ligne en faisant la chose suivante. D'une part on dit que l'on ne veut pas passer à la ligne suivante lors du premier affichage, à l'aide de : 
\begin{verbatim}
write(*,'(a)', advance='no') 'Reading unformatted outputs...'
\end{verbatim}

Puis on dit (ou pas) que l'on souhaite commencer l'écriture au début de la ligne en utilisant le retour chariot \verb|\r| qui se code en fortran \verb|achar(13)|. 

Ainsi, on peut coder de deux manière équivalente l'ajout d'un 'Done' à la ligne du dessus, soit en la réécrivant : 
\begin{verbatim}
write(*,'(a)', advance='no') 'Reading unformatted outputs...'
write(*,'(a,a)') achar(13), 'Reading unformatted outputs... Done'
\end{verbatim}
soit en ajoutant simplement à la suite de la ligne : 
\begin{verbatim}
write(*,'(a)', advance='no') 'Reading unformatted outputs...'
write(*,'(a)') ' Done'
\end{verbatim}

Dans le cas présent, la dernière solution est la plus logique, mais pour afficher une barre de progression avec un pourcentage, il faut réécrire toute la ligne.
...
\section{Documentation automatique}\index{doxygen}
...
\begin{remarque}
Pour continuer une ligne de description pour une fonction qui n'en laisse pas la possibilitÃ©, on peut faire : 
\begin{verbatim}
real(double_precision), intent(inout) :: time !< [in,out] description
!! \n description continues on another line
\end{verbatim}
\end{remarque}
...
On peut aussi créer des pointeurs vers des fonctions. Pour plus de détails voir \refsec{sec:pointeur-fonction}.
