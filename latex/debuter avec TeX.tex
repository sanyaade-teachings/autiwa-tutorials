...
\begin{Huge}\TeX\end{Huge}\hfill\includegraphics[width=0.15\textwidth]{pdf/logo-ubuntu.pdf}\hfill\begin{Huge}\LaTeXe\end{Huge}\\[1cm]
...
\begin{figure}[htb]
\centering
\subfloat[1\ier{} figure]{\label{fig:figure1}\includegraphics[width=0.2\textwidth]{pdf/logo-ubuntu.pdf}}\hfill
\subfloat[2\ieme{} figure]{\label{fig:figure2}\includegraphics[width=0.2\textwidth]{pdf/logo-debian.pdf}}
...
la séquence \verb|{\large| et le \verb|}| final seront imprimés tels quels, tandis que le \verb|\emph{Texte en corps plus| \verb|grand}| sera interprété (on aura donc le contenu du bloc en italique). Il faut évidemment choisir un caractère d'échappement qui ne sera pas dans le code.

On peut mettre du code au sein d'un texte, avec la commande \verb|\lstinlinec\dots c|. Le c indique un caractère qui marque le début et la fin du code ; il peut être choisi arbitrairement, mais ne doit évidemment pas faire partie du code. Par exemple

Pour mettre du texte en emphase,
on utilise la commande \verb|\lstinline-\emph-|.
...
\end{lstlisting}
\end{verbatim}
%\end{lstlisting}Cette ligne est uniquement là pour corriger un "bug" de coloration syntaxique de Kile. Elle ne sert à rien dans l'absolu.
...
\section{Présentation avec \LaTeX et beamer}\index{package!beamer}
...
\subsection{Animations élémentaire}
...
\subsection{Faire fonctionner natbib}\index[package!natbib]{natbib}\label{sec:beamer_natbib}
Insérer immédiatement après \verb|\bibliography{}| :
\begin{verbatim}
\def\newblock{}
\end{verbatim}
