...
nombre de planètes qui le composent initialement. Les excentricités sont représentées par les barres d'erreurs autour des points
symbolisant les planètes, dont le diamètre est proportionnel au rayon (en considérant qu'elles ont toutes la même
densité).}\label{fig:MMR_number}
\end{figure}

%TODO présenter que si on augmente la force de migration ou le nombre d'embryons on augmente la compression (visible avec un
%diagramme des resonnances)
