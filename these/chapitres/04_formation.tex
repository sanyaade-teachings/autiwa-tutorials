...
\subsection{Discussion}
Quand les embryons sont plus petits qu'une certaine masse seuil dépendant des propriétés du disque, la migration est systématiquement vers l'intérieur. Un système compact de planètes qui grossissent par colision se forme alors au bord interne qui retient ce système par le fort couple positif qui s'exerce juste avant le bord interne en raison de la forte décroissante de la densité de surface. 
...
La seule différence entre le cas système compact et le cas planète géante est le timing. Dans le cas de la planète géante, l'embryon qui en est à l'origine va très rapidement grossir. Ici la limite est qu'il faut qu'un corps possède plus de $5\mearth$ avant de migrer en dessous de $1\unit{UA}$. Les planètes qui ne répondent pas à ses critères migreront inexorablement vers le bord interne. 
...
Ensuite, \cite{terquem2007migration} montrent que la formation de systèmes compacts est possible. Ici, le modèle que nous avons repris est très similaire à leur modèle, à ceci près que nous avons modélisé la migration de manière consistante avec le disque (avec possibilité de couple positif et négatif en fonction de la masse et de la position de la planète). 
