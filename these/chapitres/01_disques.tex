...
Dans ce paramètre, $\pi G \Sigma$ représente l'importance de la gravité. La vitesse du son $c_{s}$ est liée à la pression thermique ; la \gras[fréquence!epicyclique@épicyclique]{fréquence épicyclique} $\kappa$ détermine quant à elle la force du cisaillement dans le disque.
...
\begin{align}
\inv{\rho}\pd{P}{z} &= - \vect{g}.\hat{e}_z\\
...
il vient
\begin{align}
\pd{\rho}{z} &= -\frac{\mu m_H\Omega^2}{k_B T}\rho z
\end{align}
...
%TODO parler de la photoévaporation, l'introduire par rapport à la dissipation visqueuse
...
%TODO donner la formule analytique du couple de Lindblad
...
Sans considérer de grandeur thermodynamique particulière, il convient de définir certains temps caractéristiques afin de comprendre l'origine du couple de corotation. \reffig*{fig:corotation_orbits} représente schématiquement les 3 temps principaux mis en jeux. 
...
Pour la grandeur physique considérée est la vortensité (ou vorticité spécifique), seul $t_\text{visc}$ est à prendre en compte. Quand c'est l'entropie, les deux temps $t_\text{visc}$ et $t_\text{rad}$ sont importants. \reffig*{fig:corotation_orbits} récapitule les temps mis en jeux et à quoi ils correspondent.
...
Si le temps de diffusion $t_\text{diff}$ est plus grand que le temps de libration $t_\text{lib}$, alors une particule fluide verra ses propriétés physique converger vers une valeur moyenne qui restera plus ou moins constante au cours d'une orbite. Le couple de corotation va ainsi saturer, celui-ci étant issu de différence de propriétés physiques entre les différentes zones de l'orbite fer-à-cheval. Afin que le couple de corotation ne sature pas, il faut que le temps de diffusion $t_\text{diff}$ soit suffisamment court pour que l'état de l'élément fluide soit remis à zéro avant d'atteindre de nouveau la zone de U-turn. 

Si le temps de diffusion $t_\text{diff}$ est plus court que le temps pour effectuer le demi tour $t_\text{U-turn}$, alors la particule fluide aura le temps de s'équilibrer avec le disque avant même d'avoir fait un demi tour. Il n'y aura donc pas de différence avec le reste du disque, et donc pas de couple issu de différence de propriété physique à la zone de corotation.
...
%TODO réécrire ou supprimer cette partie
La migration de type III est un type de migration intermédiaire aux migrations de type I et II. Le principe est que la planète commence à migrer par type I, tout en accrétant du gaz. À mesure que la masse de la planète augmente, elle creuse petit à petit un gap et la migration devient une migration de type II \citep{masset2003runaway}. 

%TODO dire que ça s'applique plutôt à des masses de type jovien. Regarder dans masset et papaloizou 2003. Il faut préciser ça, parler des conditions etc...


\subsection{L'amortissement de l'excentricité}%circularisation
Je ne vais pas chercher à détailler ici le mécanisme mathématiquement. Le principe de la décomposition en série de Fourier détaillé pour le calcul des couples de Lindblad et Corotation est toujours le même. Mais au delà des couples, il y a un effet subtil sur les paramètres orbitaux de la planète que je vais essayer de détailler avec une image pour l'excentricité et l'inclinaison. 

Quand une planète possède une excentricité, elle oscille autour de sa position moyenne qu'est son demi-grand axe. En conséquence, elle passe dans des zones du disques où la vitesse du disque est légèrement supérieure ou légèrement inférieure à sa propre vitesse orbitale. Ainsi, quand la planète est en dessous de son demi-grand axe, elle est dans une zone où la vitesse orbitale est plus grande que sa vitesse, cette dernière est donc accélérée. 

À l'inverse, quand la planète est dans les parties externes du disques, proche de son apo-astre, la rotation du disque est plus faible, elle est donc freinée. Ces effets ont tendance à réduire l'excentricité d'une planète et à circulariser son orbite, sur des temps qui vont de la centaine au millier d'années dépendant des propriétés du disque.


\subsection{L'amortissement de l'inclinaison}%coplanarisation
Le principe est le même pour l'inclinaison d'une planète. Au cours de l'orbite, la planète va passer tantôt au dessus ou en dessous du disque. L'attraction gravitationnelle du gaz sur la planète n'étant plus symétrique, elle va avoir tendance à ramener la planète dans le plan du disque. 
