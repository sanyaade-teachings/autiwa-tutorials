...
Le moment cinétique $\vect{J}$ d'une particule de masse $m$ est défini par :
\begin{align}
...
Prenons le cas d'un anneau de gaz en rotation autour d'une étoile, entre les positions $R$ et $R+\Delta R$ \reffig{fig:disk_ring}.
...
Son moment cinétique est alors défini par :
\begin{align}
\vect{J_a} &= \vect{R} \wedge (m_a\vect{v(R)}) \nonumber\\
&= m_a \cdot R \cdot R\Omega(R)\nonumber\\
&= 2\pi R \Delta R \Sigma(R)\cdot R \cdot R\Omega(R)\nonumber\\
\vect{J_a} &= 2\pi R^3 \Delta R \Sigma(R)\Omega(R)
\end{align}
où $\Sigma$ et $\Omega$ sont la densité de surface et la vitesse angulaire du gaz à la position $R$ dans le disque.

\bigskip

Soit $v_r$ la vitesse radiale du gaz. Cette vitesse est responsable d'un certain taux d'accrétion du gaz du disque sur l'étoile centrale. On cherche maintenant à modéliser cette accrétion pour le bilan de moment cinétique sur l'anneau.
...
Au bord interne $R$, par unité de temps, la masse sortant de l'anneau est :
\begin{align}
\dif M(R) &= 2\pi R \cdot v_r(R) * \Sigma(R)
\end{align}
En effet, en multipliant la circonférence de l'anneau par la vitesse, on obtient une sorte de surface par unité de temps qui représente ce qui sort de la frontière virtuelle représentée par l'anneau en $r=R$. De même on a : 
\begin{align}
...
\dif J(R) &= \dif M(R) \cdot R^2\Omega(R)\\
\dif J(R+\Delta R) &= \vect{r} \wedge \left(\dif M(R+\Delta R) \vect{v}(R+\Delta R)\right)\nonumber\\
\dif J(R+\Delta R) &= \dif M(R+\Delta R) \cdot \left(R+\Delta R\right)^2\Omega(R+\Delta R)
\end{align}
\end{subequations}

\bigskip

À ceci s'ajoute la variation de moment cinétique induite par la friction entre anneaux concentriques, en d'autres termes, dus à la viscosité du disque. 

Cette variation de moment cinétique est représentée sous la forme d'un couple exercé par les anneaux internes et externes à celui considéré. 
...
\vect{F_\text{out}}(R+\Delta R) &= 2\pi R * \dif F_\text{vis}(R+\Delta R) = 2\pi\nu \Sigma (R+\Delta R)**2 \dod{\Omega}{r}(R+\Delta R) \cdot (- \hat{e}_\theta)
\end{align}
\end{subequations}
...
&= 2\pi\nu \Sigma R**3 \dod{\Omega}{r}(R) \hat{e}_z\\
\vect{\Gamma_\text{out}} &= (R+\Delta R)\hat{e}_r\wedge\vect{F_\text{out}}\nonumber\\
&= - 2\pi\nu \Sigma (R+\Delta R)**3 \dod{\Omega}{r}(R+\Delta R) \hat{e}_z
\end{align}

\bigskip

On fait maintenant un bilan des variations de moment angulaire pour l'anneau de gaz. Pour celà on dit que la variation de moment angulaire (que l'on écrit en dérivant $J_a(t)$) est égale au moment angulaire qui sort de l'anneau, plus celui qui rentre, moins les deux couples visqueux des anneaux adjacents. Ce qui donne : 
\begin{align}
\dod{J_a}{t} &= \dif J(R+\Delta R) - \dif J(R) + \Gamma_\text{in} + \Gamma_\text{out}
\end{align}
%TODO fini ça
