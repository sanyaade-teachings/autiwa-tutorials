...
Étant donné que j'ai implémenté les interactions disque-planète à partir de modèles qualibrés sur des simulations hydrodynamiques, je ne vais pas redémontrer ici les formules analytiques, mais plutôt m'attacher à présenter, avec les mains, le principe de chacun des phénomènes physiques mis en jeux.
...
\begin{enumerate}
\item Il y a un phénomène d'auto-interaction. Une onde de densité se forme par la cohérence des excentricité et orientation d'orbites excentriques \reffig{fig:spiral_arms}. Les excentricités et orientation des orbites et étoiles sont quant à elles soumises à la perturbation gravitationnelle induite par l'onde de densité. 
...
\subsection{Migration des planètes de faible masse}
...
Pour l'interaction entre une planète et un disque protoplanétaire, c'est exactement le même principe. Le potentiel de la planète excite les excentricités des éléments fluides voisins jusqu'à former une onde de densité autour de la planète. La différence principale est que le perturbateur est un potentiel gravitationnel tournant.

%TODO j'en suis là. Une idée, c'est que la résonnance se passe à un endroit précis du disque, mais ensuite, par viscosité, ça se propage dans le reste du disque, faut que je vois ça avec arnaud.
...
Afin de le voir, il faut décomposer le potentiel gravitationnel de la planète en série de Fourier. Le potentiel de la planète dans le référentiel tournant à la vitesse angulaire $\Omega_{p}$ et lié à l'étoile centrale vaut alors :
\begin{align}
\phi(r, \theta, t) &= \sum _{n} \phi_{n}(r) \cos\left(m(\theta - \Omega_{p} t)\right)\notag\\
&= \sum _{n} \phi_{n}(r) \cos\left(m(\Omega - \Omega_{p}) t)\right)
\end{align}
...
Comme son nom l'indique, c'est un couple exercé par les zones coorbitales entourant la planète sur cette dernière.

%TODO rajouter la figure d'une orbite fer à cheval, voire en plus un schéma de la zone pour représenter les temps (visqueux, u-turn etc) et les rapports entre les temps caractéristiques). Penser aussi à supprimer les références à la section de 'resume.tex' que je n'ai pas recopié ici.
Par exemple, si on considère plusieurs particules fluides en orbite fer-à-cheval (voir \refsec{sec:region-coorbitale}) avec la planète. Un couple sur la planète va apparaître sur le long de cette orbite si la densité de gaz rencontrée n'est pas uniforme --- Une densité non uniforme implique une attraction gravitationnelle non uniforme azimutalement dans cette région. C'est le cas par défaut, mais sans mécanismes pour diffuser la densité, le \gras[couple!co-orbital]{couple co-orbital} s'estompe. À mesure que la densité s'uniformise, on dit que le couple  \gras[couple!saturé]{sature}.
