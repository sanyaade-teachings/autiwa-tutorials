...
Un paramètre crucial des modèles de disques protoplanétaires est l'opacité du disque. Cette dernière, qui représente l'absorption du rayonnement incident par une cellule de gaz dépend de la composition chimique de la poussière, source principale d'opacité dans le disque (sauf quand la température devient suffisamment importante pour que la totalité de la poussière se sublime, généralement au delà de $1500\unit{K}$.

La source dominante d'opacité est la poussière et les différentes températures/pression auxquelles certaines espèces se subliment auront donc des conséquences importantes sur le profil d'opacité. 

L'opacité dépend de plus de la longueur d'onde, les raies d'absorptions n'étant pas uniformément réparties sur toute la gamme de longueur d'onde. Pour autant, dans les modèles de disque on considère généralement des opacités dites \og grises\fg qui sont intégrées sur tout le spectre. On voit donc ici qu'en fonction du modèle choisi pour l'opacité, nous aurons des opacités différentes, qui vont se répercuter sur d'autres paramètres du disque, notamment sa température, et par extension toutes les grandeurs qui dépendent de la température. 


%TODO parler des opacités de rosseland et planck. Je ne sais pas les différences et je n'ai pas trouvé
