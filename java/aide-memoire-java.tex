...
Il y a deux principaux types de variables. Tout d'abord les variables de classes, qui sont définies dans la classe, avec l'attribut \gras{static}. Cette variable sera commune à toutes les instances de la classe. 
\begin{remarque}
Pour accéder à une \gras{variable de classe} \textbf{var} (de la classe \textbf{Ville} par exemple) à l'intérieur même de celle-ci (ou une de ses instances), on y accède via :
\begin{lstlisting}[language=java]
Ville.var
\end{lstlisting}
\end{remarque}
...
Il y a ensuite les variables d'instance, qui sont définies dans la classe. Cette variable \textbf{var} est accessible, à l'intérieur de la classe via \textbf{this.var} où \gras{this} désigne l'instance de classe. 
\begin{remarque}
En effet, en règle générale, on appelle des méthodes ou des paramètres grâce à notre variable qui nous sert de référence, celle-ci suivie de l'opérateur ".", puis du nom de la dite méthode/variable.
\begin{lstlisting}[{language=java}]
...
Le mot clé \gras{final} empêche la redéfinition, l'héritage ou la surcharge d'une méthode ou classe. Pour plus de détails, voir \refsec{sec:surcharge}.

\bigskip

Le mot-clé abstract permet de définir des classes abstraites \refsec{sec:classe_abstraite} ou des méthodes abstraites \refsec{sec:methode_abstraite}. Ces dernières sont utiles dans le cas d'un héritage, quand on souhaite uniquement définir les outils communs, sans qu'ils puissent être utilisés dans une classe parente commune.

\bigskip

\gras{interface} permet de considérer une classe comme une interface. Plus de détails au \refsec{sec:interface}.

%TODO autres mots clés? static?
...
\subsection{Classes dérivées d'une autre classe : Héritage}\index{héritage}
...
Le mot-clé \gras{super()} permet de modifier les opérateurs existants (au lieu de les remplacer). ça signifie concrêtement que \gras{super()} va exécuter la version parente de la méthode en lieu et place du mot-clé. Ceci fonctionne bien entendu pour le constructeur, ce qui permet d'initialiser la classe parente de manière normale, tout en rajoutant l'initialisation des variables propre à la classe courante : 
\begin{lstlisting}[language=java]
...
Toutes les \textbf{méthodes} d'une \textbf{classe} n'utilisant que des \gras{variables de classes} doivent être déclarées \gras{static} ! On les appelle des \textbf{méthodes de classes} car elles sont globales à toutes vos instances !

\begin{remarque}
Par contre ceci n'est plus vrai si une méthode utilise des variables d'instances et des variables de classes...
\end{remarque}

L'\gras{accesseur} d'une \gras{variable de classe} doit aussi être déclarée \gras{static}, c'est une règle !
...
En déclarant le mot-clé \gras{final}, on empêche la surchage de méthode. Ces méthodes sont figées et vous ne pourrez \textit{jamais} redéfinir une méthode déclarée \gras{final}. Un exemple de ce type de méthode est la méthode \textbf{getClass()} de la classe \textbf{Object}.
...
\begin{remarque}
L'utilisation d'\gras[interface]{interfaces} permet aussi d'utiliser le polymorphisme dans des classes non issues de la même classe mère (voir \refsec{sec:interface}).%TODO faire la section interface pour que le lien fonctionne
\end{remarque}
