...
\end{lstlisting}    \textsl{}

\subsection{Bien gérer les variables}
...
Il y a deux principaux types de variables. Tout d'abord les \gras[variable!de classe]{variables de classes}, qui sont définies dans la classe, avec l'attribut \gras{static}. Cette variable sera commune à toutes les instances de la classe. 
\begin{remarque}
...
Il y a ensuite les \gras[variable!d'instance]{variables d'instance}, qui sont définies dans la classe. Cette variable \textbf{var} est accessible, à l'intérieur de la classe via \textbf{this.var} où \gras{this} désigne l'instance de classe. 
...
\gras{static} permet de définir une variable comme constante ou une méthode comme immuable. Je n'ai pas bien compris le détail, mais en gros, ça permet de définir des variables de classes. Pour le reste, lire la doc. %TODO expliquer mieux 'static'
...
\subsection{Classes dérivées d'une autre classe : Héritage}\index{héritage}\label{sec:heritage}
...
Pour plus de détails sur \gras{super()} voir \refsec{sec:surcharge}.
